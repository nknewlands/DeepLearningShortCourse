...
from tensorflow.keras.layers import Conv1D, Dense, MaxPooling1D, Flatten, GRU
...
################################################################################ 
# MODEL DEFINITION (GRU, one layer)                                            #
################################################################################
model = Sequential()
model.add(GRU(32, dropout=0.01, return_sequences=True, input_shape=(train_features.shape[1], 40)))
model.add(Flatten())
...
################################################################################
#
# 1. This is a modification of the 1D CNN approach with memory cells (GRU)
# 2. A modification could imply the use of LSTM and longer sequence lengths.